...
document.getElementById("topbar").innerHTML = `
  <div class="topbar__inner">
    ...
    <div class="search">
      ...
      <input type="search" id="q" placeholder="상호·호수·전화 (예: B110, 약국)" autocomplete="off" />
...
function buildChips(host, keys, labelOf) {
  ...
  host.addEventListener("click", (e) => {
    ...
    if (host === wingChips) wing = btn.dataset.key;
...
document.getElementById("focus-ho").addEventListener("click", () => {
  focus = focus === "ho" ? "" : "ho";
  expandAll = null;
  update();
});
document.getElementById("goto-pending").addEventListener("click", goPending);
document.getElementById("survey-toggle").addEventListener("click", () => {
  survey = !survey;
  expandAll = null;
  update();
});
...
function shopTable(list, mode) {
  ...
  const rows = sortShops(list).map((s) => {
    const phone = s.phone
      ? `<a class="tel tel-btn" href="${telHref(s.phone)}">${mark(s.phone, q)}</a>`
    ...
    return `<tr${rowCls ? ` class="${rowCls}"` : ""}>
      <td data-label="상호" class="cell-name">${mark(s.name, q)}${sign}${leaf}${dup}${hoEx}${report}</td>
      <td data-label="호수">${ho}</td>
...
function update() {
  ...
  document.getElementById("ho-missing-n").textContent = String(hoMissing);
  document.getElementById("pending-n").textContent = String(PENDING.length);
  document.getElementById("focus-ho").classList.toggle("active", focus === "ho");
  document.getElementById("focus-ho").setAttribute("aria-pressed", focus === "ho" ? "true" : "false");
  document.getElementById("survey-toggle").classList.toggle("active", survey);
  document.getElementById("survey-toggle").setAttribute("aria-pressed", survey ? "true" : "false");
  ...
  const open = expandAll === null
    ? Boolean(q) || focus === "ho" || survey || shops.length <= COMPACT_LIMIT
    : expandAll;
  ...
  if (q) compact.push(`"${q}"`);
  filterCompact.textContent = compact.length
    ? `${compact.join(" · ")} · ${shops.length}곳`
    : "";
  filterCompact.hidden = !compact.length;

  const pendingFab = document.getElementById("pending-fab");
  if (pendingFab) {
    pendingFab.hidden = pendingOpen || !pendingAll.length || survey;
